...
export default function StandingsPreview({ standings, seasonName }: StandingsPreviewProps) {
  // Classic default standings matching the requested 30th season stats pattern
  const defaultStandings: StandingRow[] = [
    { position: 1, team: 'VITOROG PROMET', isPirli: false, played: 14, wins: 12, draws: 1, losses: 1, goalsFor: 70, goalsAgainst: 30, goalDifference: 45, points: 37 },
    { position: 2, team: 'KORVEX', isPirli: false, played: 14, wins: 11, draws: 0, losses: 3, goalsFor: 65, goalsAgainst: 41, goalDifference: 24, points: 33 },
    { position: 3, team: 'CODOLIS', isPirli: false, played: 14, wins: 9, draws: 1, losses: 4, goalsFor: 50, goalsAgainst: 38, goalDifference: 12, points: 28 },
    { position: 4, team: 'GRADITELJ NS', isPirli: false, played: 14, wins: 9, draws: 0, losses: 5, goalsFor: 61, goalsAgainst: 52, goalDifference: 9, points: 27 },
    { position: 7, team: 'KMF PIRLI', isPirli: true, played: 14, wins: 8, draws: 0, losses: 6, goalsFor: 58, goalsAgainst: 62, goalDifference: -4, points: 24 },
  ];
  ...
  return (
    <div id="home-standings-preview" className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Link href="/standings" className="text-xs text-blue-400 font-bold hover:underline flex items-center gap-0.5">
          ...
          <table className="w-full min-w-[500px] border-collapse text-left text-xs text-slate-300">
            <thead>
              <tr className="bg-[#111827] text-[10px] text-slate-400 font-extrabold tracking-wider uppercase border-b border-slate-800 font-mono">
                ...
                <th className="px-2 py-3 text-center w-10 text-yellow-500">N</th>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {rows.map((row) => (
                ...
                >
                  ...
                  <td className="px-2 py-2.5 text-center font-mono">{row.wins}</td>
                  <td className="px-2 py-2.5 text-center font-mono">{row.draws}</td>
                  <td className="px-2 py-2.5 text-center font-mono">{row.losses}</td>
