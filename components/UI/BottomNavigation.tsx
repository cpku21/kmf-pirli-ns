...
import { Home, Users, Calendar, Trophy, ChevronRight, Activity } from 'lucide-react';
...
export default function BottomNavigation({ currentPath, onNavigate }: BottomNavigationProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-16 bg-[#111827] border-t border-slate-800 flex items-center justify-around px-2 z-50 max-w-md mx-auto shadow-lg pb-safe">
      {tabs.map((tab) => {
        const Icon = tab.icon;
        const isActive = currentPath === tab.path || (tab.path === '/' && (currentPath === '' || currentPath === '/home'));
          ...
          >
            <div
              className={cn(
                ...
                isActive ? "text-blue-400 bg-blue-500/10" : "text-slate-450 text-slate-400"
              )}
            >
